        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row sm:flex-wrap gap-4 sm:items-end">
          ...
          <div className="w-full sm:w-auto flex items-end space-x-2">
              <div className="relative" ref={historyRef}>
                  <button type="button" onClick={() => setShowHistory(s => !s)} disabled={disabled} title={T.viewHistory} className="h-[46px] w-[46px] bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-500 dark:text-gray-300 font-bold rounded-xl transition-all flex items-center justify-center border border-gray-200 dark:border-dark-border shadow-sm active:scale-90"><HistoryIcon className="h-5 w-5" /></button>
                  {showHistory && (
        ...
        <div className="flex items-center justify-between">
            <div className="flex flex-wrap items-center gap-2">
                <div className="relative" ref={savedWorkMenuRef}>
                    <button onClick={() => setIsSavedWorkMenuOpen(!isSavedWorkMenuOpen)} disabled={disabled} title={T.accessSavedWork} className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-200 font-bold py-2.5 px-4 rounded-xl transition-all flex items-center justify-center space-x-2 text-sm border border-gray-200 dark:border-dark-border shadow-sm active:scale-95"><Library className="h-4 w-4" /><span className="hidden sm:inline">{T.savedWorkButton}</span><ChevronDown className={`h-3 w-3 transition-transform ${isSavedWorkMenuOpen ? 'rotate-180' : ''}`} /></button>
                    {isSavedWorkMenuOpen && (
                ...
                <button onClick={onOpenClinicalTools} disabled={disabled} title={T.clinicalToolsTooltip} className="bg-indigo-50 dark:bg-indigo-900/20 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 font-bold py-2.5 px-4 rounded-xl transition-all flex items-center justify-center space-x-2 text-sm border border-indigo-100 dark:border-indigo-900/50 shadow-sm active:scale-95"><Calculator className="h-4 w-4" /><span className="hidden sm:inline">{T.calculatorsLabel}</span></button>
            </div>
            <div className="lg:hidden"><div className="flex items-center bg-gray-100 dark:bg-slate-800 p-1 rounded-xl transition-colors"><button onClick={() => onSetMobileView('case')} title={T.textualViewTooltip} className={`p-2 rounded-lg transition-all ${mobileView === 'case' ? 'bg-white dark:bg-slate-700 text-brand-blue dark:text-brand-blue-light shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}><FileText className="h-5 w-5" /></button><button onClick={() => onSetMobileView('map')} title={T.knowledgeMapViewTooltip} className={`p-2 rounded-lg transition-all ${mobileView === 'map' ? 'bg-white dark:bg-slate-700 text-brand-blue dark:text-brand-blue-light shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}><Network className="h-5 w-5" /></button></div></div>
            <button onClick={onGenerateNew} disabled={disabled} title={T.resetAppTooltip} className="bg-brand-blue hover:bg-blue-800 text-white font-black py-2.5 px-5 rounded-xl transition-all flex items-center justify-center space-x-2 shadow-lg shadow-brand-blue/20 active:scale-95"><Plus className="h-5 w-5" /><span className="hidden sm:inline">{T.resetAppLabel}</span></button>
        </div>
      )}
    </div>
  );
