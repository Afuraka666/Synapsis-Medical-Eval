 ...
 * 
 * Renders an interactive process flow diagram using D3.js.
 * 
 * Visualization Standard Compliance:
 * - Uses D3.js for SVG-based rendering.
 * - Nodes are interactive (drag, click).
...
export const InteractiveDiagram: React.FC<InteractiveDiagramProps> = ({ data, id }) => {
  ...
  const initDiagram = useCallback(() => {
    ...
    const svg = d3.select(svgRef.current)
      .attr('role', 'img')
      .attr('aria-label', `Interactive diagram: ${data.nodes.length} nodes and ${data.links.length} connections`);
    
    svg.selectAll("*").remove();

    svg.append('title').text("Interactive Process Flow Diagram");
    svg.append('desc').text("An interactive SVG diagram showing relationships between concepts. Nodes can be dragged and clicked for more information.");
